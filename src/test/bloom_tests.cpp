...
{
    // TESR_TODO generate new test data
#ifdef TESR_BREAK_TEST
    ...
    //! TESR_TODO: use an array of characters(bytes) to represent the tnx 
    ...
    // Note: Bitcoin addresses use the non-segwit address and pubkey addresses. whereas Tesseract tests use segwit transactions and script addresses
    ...
    //! TESR_TODO: the following signature found in txinwitness does not pass the test 
    ...
    //! TESR_TODO: the following pkey found in txinwitness does not pass the test  
...
BOOST_AUTO_TEST_CASE(merkle_block_1)
{
    ...
    CBlock block = getBlockTesseract();
    CBloomFilter filter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    // Match the last transaction
