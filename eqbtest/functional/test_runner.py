...
def main():
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and tesseractd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "tesseractd"]) is not None:
            print("%sWARNING!%s There is already an tesseractd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + dir_separator + 'src' + dir_separator + 'tesseractd' + exeext
        os.environ["BITCOINCLI"] = build_dir + dir_separator + 'src' + dir_separator + 'tesseract-cli' + exeext
